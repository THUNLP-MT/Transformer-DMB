...
class AttentionSubLayer(modules.Module):
    ...
    def __init__(self, params, name="attention"):
        ...
        with utils.scope(name):
            self.gate = modules.Affine(params.hidden_size, params.n)
            self.attention = modules.dmb.MultiHeadAttention(
                params.hidden_size, params.num_heads, params.attention_dropout,
                n=params.n, shared_private=params.shared_private)
            self.layer_norm = modules.LayerNorm(params.hidden_size)

    def forward(self, x, bias, memory=None, state=None):
        logits = self.gate(x)
        probs = torch.softmax(logits, dim=-1)
        partitions = torch.argmax(probs, dim=-1)

        utils.add_to_collection("name", self.name)

        if memory is not None:
            kv_logits = self.gate(memory)
            kv_probs = torch.softmax(kv_logits, dim=-1)
            kv_partitions = torch.argmax(kv_probs, dim=-1)
            partitions = (partitions, kv_partitions)
            utils.add_to_collection("gate", (probs, kv_probs))
        else:
            utils.add_to_collection("gate", probs)
        ...
        if self.training or state is None:
            y = self.attention(y, bias, memory, None, partitions=partitions)
        else:
            kv = [state["k"], state["v"]]
...
class FFNSubLayer(modules.Module):
    ...
    def __init__(self, params, dtype=None, name="ffn_layer"):
        ...
        with utils.scope(name):
            self.gate = modules.Affine(params.hidden_size, params.n)
            self.ffn_layer = modules.dmb.FeedForward(
    ...
    def forward(self, x):
        ...
        y = self.ffn_layer(y, partitions=partitions)
        y = nn.functional.dropout(y, self.dropout, self.training)
...
class TransformerDecoderLayer(modules.Module):
    ...
    def __init__(self, params, name="layer"):
        ...
        with utils.scope(name):
            self.self_attention = AttentionSubLayer(params,
                                                    name="self_attention")
            self.encdec_attention = AttentionSubLayer(
                params, name="encdec_attention")
            self.feed_forward = FFNSubLayer(params)
...
class Transformer(modules.Module):
    ...
    def forward(self, features, labels, mode="train", level="sentence"):
        ...
        names = utils.get_collection("name")
        gates = utils.get_collection("gate")
        ...
        for name, gate in zip(names, gates):
            ...
            if not isinstance(gate, tuple):
                cv_loss, ent_loss = self.compute_loss(gate, mask)
                cv_losses.append(cv_loss)
                ent_losses.append(ent_loss)
            else:
                q_gate, kv_gate = gate
                cv_loss, ent_loss = self.compute_loss(
                    q_gate, features["target_mask"])
                cv_losses.append(cv_loss)
                ent_losses.append(ent_loss)

                cv_loss, ent_loss = self.compute_loss(
                    kv_gate, features["source_mask"])
                cv_losses.append(cv_loss)
                ent_losses.append(ent_loss)

        cv_loss = sum(cv_losses) / len(cv_losses)
        ent_loss = sum(ent_losses) / len(ent_losses)

        loss = loss + self.params.alpha * (cv_loss + ent_loss)

        return (torch.sum(loss * mask) / torch.sum(mask)).to(logits)

    def compute_loss(self, gate, mask=None):
        cv_loss = self.cv_squared(gate, mask)
        ent_loss = self.entropy(gate, mask)

        return cv_loss, ent_loss

    def cv_squared(self, gate, mask, epsilon=1e-10):
        # coefficient of variance: stddev / mean
        n = torch.sum(mask)
        # [N]
        importance = torch.sum(gate * mask[:, :, None],
            [i for i in range(gate.ndim - 1)]) / n
        mean = torch.mean(importance)
        variance = torch.mean((importance - mean) ** 2)
        return variance / (mean ** 2 + epsilon)

    def entropy(self, gate, mask, epsilon=1e-10):
        # [batch, length, N] -> [batch, length]
        ent = torch.sum(gate * torch.log(gate + epsilon), -1)
        return -torch.sum(ent * mask) / torch.sum(mask)
