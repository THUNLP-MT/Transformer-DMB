...
class AttentionSubLayer(modules.Module):
    ...
    def __init__(self, params, name="attention"):
        ...
        with utils.scope(name):
            ...
            self.attention = modules.moe.MultiHeadAttention(
    ...
    def forward(self, x, bias, memory=None, state=None):
        if memory is None:
            logits = self.gate(x)
            gates = ops.top_k_softmax(logits, self.k, self.n)
        else:
            q_logits = self.q_gate(x)
            q_gates = ops.top_k_softmax(q_logits, self.k, self.n)
            kv_logits = self.kv_gate(memory)
            kv_gates = ops.top_k_softmax(kv_logits, self.k, self.n)
            gates = (q_gates, kv_gates)

        utils.add_to_collection("name", self.name)
        utils.add_to_collection("gate", gates)
        ...
        if self.training or state is None:
            y = self.attention(y, bias, memory, None, gates=gates)
        else:
            kv = [state["k"], state["v"]]
...
class FFNSubLayer(modules.Module):
    ...
    def __init__(self, params, dtype=None, name="ffn_layer"):
        ...
        with utils.scope(name):
            self.gate = modules.Affine(params.hidden_size, params.n)
            self.ffn_layer = modules.moe.FeedForward(
                params.hidden_size, params.filter_size,
                n=params.n, dropout=params.relu_dropout)
            self.layer_norm = modules.LayerNorm(params.hidden_size)
    ...
    def forward(self, x):
        ...
        y = self.ffn_layer(y, gates=gates)
        y = nn.functional.dropout(y, self.dropout, self.training)
