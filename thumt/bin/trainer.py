...
def parse_args(args=None):
    ...
    parser.add_argument("--profile", action="store_true",
                        help="Profile number of Mult-Adds")
...
def print_variables(model, pattern, log=True, total=False):
    flags = []
...
def main(args):
    ...
    if args.profile:
        features = {
            "source": torch.zeros([1, 30], dtype=torch.int64),
            "source_mask": torch.ones([1, 30], dtype=torch.float32),
            "target": torch.zeros([1, 30], dtype=torch.int64),
            "target_mask": torch.ones([1, 30], dtype=torch.float32)
        }
        labels = torch.zeros([1, 30], dtype=torch.int64)

        macs = profile_macs(model, (features, labels, "profile"))
