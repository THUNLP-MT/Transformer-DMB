...
class MultiHeadAttention(Module):
    ...
    def __init__(self, hidden_size, num_heads, dropout=0.0, n=4,
    ...
    def forward(self, query, bias, memory=None, kv=None, gates=None):
        if memory is not None:
            gates, kv_gates = gates

        q = self.q_transform(query, gates=gates)
        ...
        if memory is not None:
            ...
            k = k or self.k_transform(memory, gates=kv_gates)
            v = v or self.v_transform(memory, gates=kv_gates)
        else:
            # self-attention
            k_t = self.k_transform(query, gates=gates)
            v_t = self.v_transform(query, gates=gates)
        ...
        # combine heads
        output = self.o_transform(self.combine_heads(x), gates=gates)
            ...
            if self.n == 1:
                ...
                for i in range(self.n):
                    nn.init.xavier_uniform_(
                        self.q_transform.experts[i].weight, 2 ** -0.5)
                    nn.init.xavier_uniform_(
                        self.k_transform.experts[i].weight, 2 ** -0.5)
                    nn.init.xavier_uniform_(
                        self.v_transform.experts[i].weight, 2 ** -0.5)
                    nn.init.xavier_uniform_(
                        self.o_transform.experts[i].weight, 2 ** -0.5)
                    nn.init.constant_(self.q_transform.experts[i].bias, 0.0)
                    nn.init.constant_(self.k_transform.experts[i].bias, 0.0)
                    nn.init.constant_(self.v_transform.experts[i].bias, 0.0)
                    nn.init.constant_(self.o_transform.experts[i].bias, 0.0)
        else:
